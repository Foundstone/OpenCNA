...
def calculate_ngram(n, line, is_normalize=True):
    res = []
    if is_normalize:
        line = tools.normalize_line(line)
...
def update_n_gram_transition_counter(ngcomps, n_gram_transition_counts):
    if len(ngcomps) == 1:
        ...
        update_n_gram_transition_counter(ngcomps[1:], n_gram_transition_counts[ng1])
...
def update_log_prob(n_gram_transition_counts, total, min_prob):
    for ngc in n_gram_transition_counts.keys():
        if not isinstance(n_gram_transition_counts[ngc], dict):
            ...
            min_prob = update_log_prob(n_gram_transition_counts[ngc], total, min_prob)
    return min_prob
...
def model_entropy(file_name, epsilon=tools.EPSILON_CHARACTER):
    chars_freq = {}
    try:
...
def calculate_str_entropy(query_str, model, is_normalize=True):
    entr = 0
    if is_normalize:
        query_str = tools.normalize_line(query_str)
...
def model_n_gram(file_name, entr_model, is_filter, n=2, is_epsilon=True, epsilon=tools.EPSILON_CHARACTER, is_entr=True, entropy_threshold=ENTROPY_THRESHOLD):
    res_min_prob = 0
            ...
            with open(file_name, 'r+') as f:
                for line in f:
                    ...
                    if is_process:
                        ...
                        for ng in ngs:
                            update_n_gram_transition_counter(list(ng), n_gram_transition_counts)
                res_min_prob = 0
                if n == 1:
                    res_min_prob = normalize_log_prob(n_gram_transition_counts, res_min_prob)
                else:
                    for n_gram in n_gram_transition_counts.keys():
                        res_min_prob = normalize_log_prob(n_gram_transition_counts[n_gram], res_min_prob)
    except:
        print 'There was a problem processing the file: {}'.format(file_name)
        traceback.print_exc()
        n_gram_transition_counts = {}
...
def save_model(transition_probs, min_prob, entr_model, file_name):
    try:
        with open(file_name, 'wb') as f:
            pickle.dump({'transition_probs': transition_probs, 'entr_model': entr_model, 'min_prob': min_prob}, f)
    except:
...
def _compute_query_string_prob(query_str, model, min_prob, n=2, scaling_unseen=SCALING_FACTOR_UNSEEN):
    log_prob = 0.0
...
def compute_query_string_prob(query_str, saved_model, n=2, is_epsilon=True, epsilon=tools.EPSILON_CHARACTER, folder_separator=tools.FOLDER_SEPARATOR, scaling_unseen=SCALING_FACTOR_UNSEEN):
    sub_folder_probs = []
    ...
    for fd in query_str_sub_folders:
        ...
        prob1, prob2 = _compute_query_string_prob(fd_aux, saved_model['transition_probs'], saved_model['min_prob'], n, scaling_unseen) 
        sub_folder_probs.append((fd, prob1))
    return sub_folder_probs

def calculate_danger(query_string, saved_model, threshold, n=2, is_epsilon=True, epsilon=tools.EPSILON_CHARACTER, folder_separator=tools.FOLDER_SEPARATOR, scaling_unseen=SCALING_FACTOR_UNSEEN):
    is_positive = False
    explanation = ''
    prob_decision = 1
    sub_folder_probs = compute_query_string_prob(query_string, saved_model, n, is_epsilon, epsilon, folder_separator, scaling_unseen)
...
def calculate_mixed_danger(query_strings, models, mixed_coeffs, threshold, n=2, is_epsilon=True, epsilon=tools.EPSILON_CHARACTER, folder_separator=tools.FOLDER_SEPARATOR, scaling_unseen=SCALING_FACTOR_UNSEEN):
    explanations = set()
    res_prob = 0
    for idx, model in enumerate(models):
        res, explanation, prob_decision = calculate_danger(query_string, model, threshold, n, is_epsilon, epsilon, folder_separator, scaling_unseen)
        res_prob += mixed_coeffs[idx]*prob_decision
        explanations.add(explanation)
    return res_prob, list(explanations)

def calculate_distribution_over_corpus(folder_files_names, model, threshold, n=2, is_epsilon=True, epsilon=tools.EPSILON_CHARACTER, folder_separator=tools.FOLDER_SEPARATOR, scaling_unseen=SCALING_FACTOR_UNSEEN):
    distribution = {}
    total = 0
    total_under_threshold = 0
    total_under_threshold_with_exec = 0
    try:
        with open(folder_files_names, 'rb') as f:
            ff_names = pickle.load(f)
        total = len(ff_names)
        for fname in ff_names.keys():
            is_positive, explanation, prob_decision = calculate_danger(fname, model, threshold, n, is_epsilon, epsilon, folder_separator, scaling_unseen)
            percentage_executables = 0
            if ff_names[fname]['total_files'] != 0:
                percentage_executables = ff_names[fname]['total_execs']/float(ff_names[fname]['total_files'])
            if is_positive:
                total_under_threshold += 1
                if percentage_executables > 0:
                    total_under_threshold_with_exec += 1
            distribution[fname] = {'suspicious': is_positive, 'len': ff_names[fname]['len'], 'percent_execs': percentage_executables, 'explanation': explanation, 'prob_decision': prob_decision}
    except:
        print 'There was a problem loading the model from the file: {}'.format(folder_files_names)
        traceback.print_exc()
        distribution = {}
        total = 0
        total_under_threshold = 0
